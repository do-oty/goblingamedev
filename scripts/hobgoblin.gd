...
func configure_as_elite(progress_ratio: float = 0.0, forced_type: String = "") -> void:
	super.configure_as_elite(progress_ratio, "tank" if forced_type == "" else forced_type)
	# Keep elite hobgoblin readability: preserve stats, remove glow layers/tint.
	$AnimatedSprite2D.modulate = Color(1.0, 1.0, 1.0, 1.0)
	if elite_aura_sprite != null:
		elite_aura_sprite.queue_free()
		elite_aura_sprite = null
	if elite_sheen_sprite != null:
		elite_sheen_sprite.queue_free()
		elite_sheen_sprite = null
	if elite_magic_particles != null:
		elite_magic_particles.queue_free()
		elite_magic_particles = null
...
func _update_archetype_behavior(delta: float, direction: Vector2, distance: float) -> void:
	...
	if leap_cooldown <= 0.0 and post_leap_lockout_timer <= 0.0 and chase_timer_before_leap >= HOBGOBLIN_CHASE_TIME_BEFORE_LEAP and distance > CONTACT_RANGE * 1.55 and distance < 520.0:
		...
		var leap_distance: float = min(distance + HOBGOBLIN_LEAP_OVERSHOOT, HOBGOBLIN_LEAP_MAX_RANGE)
...
func _show_leap_indicator() -> void:
	if leap_indicator == null:
		...
		leap_indicator.polygon = _build_circle_polygon(HOBGOBLIN_LEAP_AOE_RADIUS, 30)
	...
	if leap_indicator_inner == null:
		...
		leap_indicator_inner.polygon = _build_circle_polygon(HOBGOBLIN_LEAP_AOE_RADIUS * 0.58, 24)
...
func _apply_leap_impact_damage() -> void:
	...
	landing_core.polygon = _build_circle_polygon(HOBGOBLIN_LEAP_AOE_RADIUS * 0.7, 28)
	...
	landing_ring.polygon = _build_circle_polygon(HOBGOBLIN_LEAP_AOE_RADIUS, 36)
	...
	for i in range(8):
		...
		var shard_dist: float = randf_range(HOBGOBLIN_LEAP_AOE_RADIUS * 0.45, HOBGOBLIN_LEAP_AOE_RADIUS * 0.75)
	...
	_push_nearby_enemies(impact_center, HOBGOBLIN_LEAP_AOE_RADIUS + 20.0, 340.0, 34.0, 0.28)
	...
	if target_player == null or not is_instance_valid(target_player) or not target_player.has_method("receive_damage"):
		return
	if target_player.global_position.distance_to(global_position) <= HOBGOBLIN_LEAP_AOE_RADIUS:
		var impact_damage: int = int(round(float(_get_contact_damage()) * HOBGOBLIN_LEAP_DAMAGE_MULT))
